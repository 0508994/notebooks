...
def stack_frames(stacked_frames, state, is_new_episode, stack_size=STACK_SIZE):
    ...
    if is_new_episode:
        ...
        stacked_frames = deque(
            [np.zeros((84, 84), dtype=np.int) for _ in range(stack_size)],
            maxlen=stack_size,
        )
...
def instantiate_memory(
    max_size, game, pretrain_length, possible_actions, stacked_frames
):
    """
...
def predict_action(
    explore_start, explore_stop, decay_rate, decay_step, state, actions, dqn, sess
):
    ...
    if explore_probability > exp_exp_tradeoff:
        ...
        Qs = sess.run(
            dqn.output, feed_dict={dqn.inputs_: state.reshape((1, *state.shape))}
        )
        choice = np.argmax(Qs)
...
if __name__ == "__main__":
    ...
    stacked_frames = deque(
        [np.zeros((84, 84), dtype=np.int) for _ in range(stack_size)], maxlen=stack_size
    )
    ...
    state_size = [
        ...
        84,
        4,
    ]  # Input is a stack of 4 frames (84x84x4 - width, height, channels)
    action_size = (
        game.get_available_buttons_size()
    )  # 3 possible actions: left, right, shoot
    ...
    if training:
        memory = instantiate_memory(
            memory_size, game, pretrain_length, possible_actions, stacked_frames
        )
            ...
            for episode in range(total_episodes):  # tqdm(range(total_episodes)):
                ...
                while step < max_steps:
                    ...
                    if done:
                        ...
                        print(
                            "Episode: {}\tLoss: {:.4f}\tTotalReward: {}\tExplore P: {:.4f}".format(
                                episode, loss, total_reward, explore_probability
                            )
                        )
