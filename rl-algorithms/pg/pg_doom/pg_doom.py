...
def stack_frames(stacked_frames, state, is_new_episode):
    ...
    if is_new_episode:
        ...
        stacked_frames = deque(
            [np.zeros((IMGH, IMGW), dtype=np.int) for _ in range(STACK_SIZE)],
            maxlen=STACK_SIZE,
        )
        ...
        try:
            if TRAINING:
                ...
                while epoch < NUM_EPOCHS + 1:
                    ...
                    print(
                        "Mean Reward of that batch {}".format(mean_reward_of_that_batch)
                    )
                    print(
                        "Average Reward of all training: {}".format(
                            average_reward_of_all_training
                        )
                    )
                    print(
                        "Max reward for a batch so far: {}".format(
                            maximum_reward_recorded
                        )
                    )

                    loss_, _ = sess.run(
                        [network.loss, network.train_opt],
                        feed_dict={
                            network.inputs_: states_mb.reshape(
                                (len(states_mb), IMGH, IMGW, STACK_SIZE)
                            ),
